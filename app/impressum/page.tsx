    ...
    <EditablePageShell cms={cms} isAdmin={isAdmin} mainClassName="min-h-screen bg-transparent pt-28 pb-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <LiveResizableBox boxKey="imprint.title.box" initialStyle={resolveLiveBoxStyle(liveEditor, 'imprint.title.box')} isAdmin={isAdmin} className="mb-10 text-center">
            <LiveEditableText as="h1" className="page-title" editorKey="imprint.title" initialHtml={resolveLiveHtml(liveEditor, 'imprint.title', 'Impressum')} isAdmin={isAdmin} title="Impressum Titel" />
          </LiveResizableBox>
          <div className="legal-copy content-flow body-copy">
            ...
            <LiveResizableBox boxKey="imprint.note.box" initialStyle={resolveLiveBoxStyle(liveEditor, 'imprint.note.box')} isAdmin={isAdmin} className="content-flow">
              <LiveEditableText as="p" className="body-copy text-xs" editorKey="imprint.note" initialHtml={resolveLiveHtml(liveEditor, 'imprint.note', 'Bitte ersetze die Platzhalter [Vorname Nachname], [Straße Nr.] und [PLZ Ort] mit deinen echten Angaben.')} isAdmin={isAdmin} title="Impressum Hinweis" />
            </LiveResizableBox>
          </div>
        </div>
    </EditablePageShell>
