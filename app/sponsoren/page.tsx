        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <LiveResizableBox boxKey="sponsors.intro.box" initialStyle={resolveLiveBoxStyle(liveEditor, 'sponsors.intro.box')} isAdmin={isAdmin} className="content-flow text-center mb-14 max-w-3xl mx-auto">
            <h1 className="page-title">
              ...
              <LiveEditableText as="span" className="inline" editorKey="sponsors.accentWord" initialHtml={resolveLiveHtml(liveEditor, 'sponsors.accentWord', sponsors.accentWord)} isAdmin={isAdmin} title="Sponsoren Hervorhebung" />
            </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sponsors.benefits.map((b, index) => (
                <LiveResizableBox key={b} boxKey={`sponsors.benefits.${index}.box`} initialStyle={resolveLiveBoxStyle(liveEditor, `sponsors.benefits.${index}.box`)} isAdmin={isAdmin} className="flex items-start justify-center gap-3 text-center">
                  ...
                  <LiveEditableText as="span" className="body-copy text-sm" editorKey={`sponsors.benefits.${index}`} initialHtml={resolveLiveHtml(liveEditor, `sponsors.benefits.${index}`, b)} isAdmin={isAdmin} title={`Sponsoren Vorteil ${index + 1}`} />
                </LiveResizableBox>
              ))}
            </div>
          </LiveResizableBox>
